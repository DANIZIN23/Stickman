...
class PlayState extends MusicBeatState
{
	...
	public static var fightgf:Character;
	...
	override public function create()
	{
			...
			case 'jackroomfight':
				{
						...
						micone = new FlxSprite(-50,500);
						micone.frames = Paths.getSparrowAtlas('kack/micthrowparty');
						micone.animation.addByPrefix('idle', 'one mic', 24, false);
						micone.antialiasing = true;
		...
		switch (SONG.player2)
		{
			...
			case 'spirit':
				...
				dad.y += 100;
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
			case 'jackroom' | 'jackroomfight' | 'jackroom3':
				camPos.set(dad.getGraphicMidpoint().y + 1000, dad.getGraphicMidpoint().y);
		}
		...
		if (boom)
			{
				...
				var explod:FlxSprite = new FlxSprite(-450,200).loadGraphic(Paths.image('kack/explode'));
				explod.antialiasing = true;
				explod.active = false;
				add(explod);				
			}
		if (micfight)
	...
	override function stepHit()
	{
		...
		switch (curSong)
		{
		  ...
		  switch(curStep)
		  {
			...
			case 127:
				remove(dad);
				dad = new Character(100, 100, 'jackcop-record');
				dad.y += 670;
				dad.x -= 1400;
				add(dad);
			case 296:
		  ...
		  switch(curStep)
		  {
			...
			case 192:
				...
				miconet = false;
				remove(boyfriend);
				boyfriend = new Boyfriend(770, 450,'bfguard');
				boyfriend.y += 600;
				boyfriend.x += 200;
				add(boyfriend);
				remove(dad);
			...
			case 384:
				...
				dad.y += 780;
				add(dad);
				dad.playAnim('jackpreattack',false);
			...
			case 403:
				remove(boyfriend);
				boyfriend = new Boyfriend(770, 450,'bfattack');
				boyfriend.y += 600;
				//boyfriend.x -= 40;
				add(boyfriend); 
				boyfriend.playAnim('atk', false);
			...
			/*case 404:
				remove(dad);
				dad = new Character(100,100 ,'jackgetmic');
				dad.x -= 600;
				dad.y += 800;
				add(dad);
				dad.playAnim('jackget', false);*/
			...
			case 640:
				micfight = true;
				remove(boyfriend);
				boyfriend = new Boyfriend(770, 450,'bfrevage');
				boyfriend.y += 600;
				boyfriend.x += 200; 
				add(boyfriend);
				remove(dad);
				dad = new Character(100,100,'jackhurt');
				dad.x -= 600; //lol game crash
				dad.y += 800;
				add(dad);
